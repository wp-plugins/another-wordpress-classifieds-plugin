...
class AWPCP_Payment_Transaction {
	...
	private function verify_checkout_conditions(&$errors) {
        ...
        $t = $this->get_totals();
        list($money, $credits) = array((float) $t['money'], (int) $t['credits']);
    ...
    public function verify_payment_conditions(&$errors) {
        ...
        $is_payment_required = !$this->payment_is_not_required();
        if ($is_payment_required && is_null($payment_method)) {
    ...
    public function set_status($status, &$errors) {
        ...
        if ( $allowed && in_array( self::STATUS_OPEN, $verify ) ) {
            $allowed = $allowed ? $this->verify_open_conditions($errors) : false;
        }
        if ( $allowed && in_array( self::STATUS_CHECKOUT, $verify ) ) {
            $allowed = $allowed ? $this->verify_checkout_conditions($errors) : false;
        }
        if ( $allowed && in_array( self::STATUS_PAYMENT, $verify ) ) {
            $allowed = $allowed ? $this->verify_payment_conditions($errors) : false;
        }
        if ( $allowed && in_array( self::STATUS_PAYMENT_COMPLETED, $verify ) ) {
            $allowed = $allowed ? $this->verify_payment_completed_conditions($errors) : false;
        }
        if ( $allowed && in_array( self::STATUS_COMPLETED, $verify ) ) {
            $allowed = $allowed ? $this->verify_completed_conditions($errors) : false;
        }
    ...
    public function user_has_enough_credit(&$balance=null) {
        ...
        $balance = awpcp_payments_api()->get_account_balance($this->user_id);
        $plan = awpcp_payments_api()->get_credit_plan($this->get('credit-plan'));
