...
class AWPCP_PaymentsAPI {
    ...
    public function update_account_balance($transaction) {
        if ($transaction->is_payment_completed() && $transaction->was_payment_successful()) {
    ...
    public function process_payment() {
        ...
        if (!is_null($transaction) && $transaction->is_ready_to_checkout()) {
            debug('is_ready_to_checkout:before');
            $this->set_transaction_payment_method($transaction);
            $this->process_transaction($transaction);
            debug('is_ready_to_checkout:after');

            $errors = array();

            $this->set_transaction_status_to_payment($transaction, $errors);

            debug( $transaction->payment_is_not_required(), $errors );
            ...
            debug('is_processing_payment:before');
            $this->process_transaction($transaction);
            debug('is_processing_payment:after');
